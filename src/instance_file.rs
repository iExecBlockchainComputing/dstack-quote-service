//! Startup persistence of the CVM identity to a shell-sourceable `.env` file.
//!
//! Log shippers running alongside this service inside the CVM (Fluent Bit, for
//! instance) need the dstack `instance_id` to label the records they forward.
//! Rather than giving each of them its own access to the dstack socket, this
//! service — which already talks to the guest agent — writes the identity once
//! at startup to a file on a shared volume.
//!
//! The file is written before the HTTP listener is bound, so a successful
//! `/health` response also guarantees that the file is present. Consumers can
...
pub async fn write(client: &DstackClient, cfg: &InstanceFileConfig) -> Result<()> {
    ...
    write_atomically(&cfg.path, &render_env(&info)).await
}
...
/// Renders the exported identity fields as a shell-sourceable `.env` document.
///
/// Only the fields useful as log labels are exported.
fn render_env(info: &InfoResponse) -> String {
    [
...
/// Wraps a value in single quotes so a POSIX shell can source it verbatim.
///
/// Single quotes suppress every form of expansion, so the only character needing
/// care is the single quote itself: it is closed, escaped, then reopened.
...
mod test {
    ...
    mod shell_quote {
        ...
        #[test]
        fn should_produce_output_a_posix_shell_reads_back_verbatim() {
            let value = "weird 'name' with $HOME and `backticks`";
            let script = format!("VALUE={}\nprintf '%s' \"$VALUE\"", shell_quote(value));
    ...
    mod write_atomically {
        ...
        async fn should_create_the_missing_parent_directory() {
            ...
            let path = dir.join("instance.env");

            write_atomically(&path, "INSTANCE_ID='i-1'\n")
                .await
                .expect("write must succeed");
            let written = std::fs::read_to_string(&path).expect("file must exist");
            let _ = std::fs::remove_dir_all(&dir);

            assert_eq!(written, "INSTANCE_ID='i-1'\n");
        }
        ...
        async fn should_overwrite_an_existing_file() {
            ...
            write_atomically(&path, "INSTANCE_ID='old'\n")
                .await
                .expect("first write must succeed");
            write_atomically(&path, "INSTANCE_ID='new'\n")
                .await
                .expect("second write must succeed");
            let written = std::fs::read_to_string(&path).expect("file must exist");
            let _ = std::fs::remove_dir_all(&dir);

            assert_eq!(written, "INSTANCE_ID='new'\n");
        }
    }
}
